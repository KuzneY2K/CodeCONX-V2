  ...
  <section class="row p-0 m-0 d-flex flex-row justify-content-center">
    <div class="col-md-9 col-11 p-0 m-0 mb-5">
      <form class="w-100 bg-white mt-5 p-5 rounded border border-2 elevation-5 " @submit.prevent="editAccount()">
        ...
        <!-- PFP -->
        <div class="mb-3">
          <label for="imageUrl" class="form-label">Insert URL for profile picture.</label>
          <input type="url" v-model="reqBody.picture" class="form-control p-1" id="imageUrl"
            aria-describedby="imgUrlHelp">
          <div id="imgUrlHelp" class="form-text">We only support URLs for GIFs and Images.</div>
        </div>
        <!-- COVER IMG -->
        <div class="mb-3">
          <label for="imageUrl" class="form-label">Insert URL for cover img.</label>
          <input type="url" v-model="reqBody.coverImg" class="form-control p-1" id="imageUrl"
            aria-describedby="imgUrlHelp">
          <div id="imgUrlHelp" class="form-text">We only support URLs for GIFs and Images.</div>
        </div>
        <!-- BIO -->
        <div class="mb-3">
          <label for="imageUrl" class="form-label">Insert new bio.</label>
          <input type="text" v-model="reqBody.bio" class="form-control p-1" id="imageUrl" aria-describedby="imgUrlHelp">
        </div>
        <!-- Update Cohort -->
        <div class="mb-3">
          <label for="imageUrl" class="form-label">Insert your cohort.</label>
          <input type="text" v-model="reqBody.class" class="form-control p-1" id="imageUrl" aria-describedby="imgUrlHelp">
        </div>
